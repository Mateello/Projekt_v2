...
Okno::Okno(float szerokosc, float wysokosc, std::string name)//konstruktor
{
	...
	tekst[0].setPosition(sf::Vector2f(szerokosc/2, wysokosc/4/*(masymalna iloϾ wierszy +1)*1 */));//pozycja linijki tekstu

	tekst[1].setFont(czcionka);
	tekst[1].setFillColor(sf::Color::Green);
	tekst[1].setString("Opcje");
	tekst[1].setPosition(sf::Vector2f(szerokosc / 2, wysokosc / 2/*(masymalna iloϾ wierszy +1)*2*/));//pozycja linijki tekstu

	tekst[2].setFont(czcionka);
	tekst[2].setFillColor(sf::Color::Blue);
	tekst[2].setString("Wyjscie");
	tekst[2].setPosition(sf::Vector2f(szerokosc / 2, wysokosc / 3/*(masymalna iloϾ wierszy +1)*3*/));//pozycja linijki tekstu
}
